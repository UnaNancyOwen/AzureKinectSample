...
inline void kinect::initialize_playback()
{
    ...
    // Open Playback
    K4A_RESULT_CHECK( k4a_playback_open( playback_file.generic_string().c_str(), &playback ) );

    // Get Calibration
    K4A_RESULT_CHECK( k4a_playback_get_calibration( playback, &calibration ) );
...
inline void kinect::update_frame()
{
    ...
    if( playback_file.empty() ){
        ...
        const k4a_stream_result_t result = k4a_playback_get_next_capture( playback, &capture );
        if( result == K4A_STREAM_RESULT_FAILED ){
            throw std::runtime_error( "Failed to get capture from playback!" );
        }
...
inline void kinect::update_transformation()
{
    ...
    if( playback_file.empty() ){
        ...
        K4A_RESULT_CHECK( k4a_image_create_from_buffer( K4A_IMAGE_FORMAT_COLOR_BGRA32, color.cols, color.rows, static_cast<int32_t>( color.step ), &color.data[0], static_cast<int32_t>( color.total() * color.elemSize() ), nullptr, nullptr, &color_image ) );
